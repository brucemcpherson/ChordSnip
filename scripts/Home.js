...
var Home = (function (home) {
  ...
  ns.initialize = function (reason) {
    ...
    if (Process.control.chord.premium) {
    ...
    Process.control.buttons.generate.addEventListener('click',function () {
      ...
      Provoke.run("Server","generateTestData",Process.control.chord.testData)
    ...
    Process.control.buttons.insert.addEventListener('click', function () {    
      ...
      ['catch']( 
        function(err) {
          App.showNotification ('not supported', err);
        }
      );
    });
    ...
    var elementer = Process.control.chord.elementer;
    ...
    Process.control.buttons.apply.addEventListener('click', function () { 
      ...
      Object.keys(Process.control.chord.store).forEach(function(d) {
        try {
          if (elems.controls[d] && elems.controls[d].checked) {
            elementer.applySettings(Process.control.chord.store[d]);
    ...
    Process.control.buttons.reset.forEach(function(d) {
      d.addEventListener('click',function() {
        ...
        Process.control.chord.store.reset[d.id.match(/resetButton_(\w+)-elem/)[1]] = null;
        d.disabled = true;
      });
    });
    ...
    Process.control.buttons.manage.addEventListener('click', function () {    
      ...
      if (elems.controls.makePermanent.checked) {
        ...
        .then (function (result) {
          Process.control.chord.store.useDocument = current;
        ...
        .then (function (result) {
          Process.control.chord.store.useUser = current;
        ...
        .then(function (result) {
          Process.control.chord.store.useDocument = null;
        ...
        .then(function (result) {
          Process.control.chord.store.useUser = null;
    ...
    var elem = Process.control.chord.elementer.getElements().controls.applyFilters;
    Process.control.watching.watcher.getWatching().domain.applyFilters =  elem && elem.checked;
    if (elem) {
